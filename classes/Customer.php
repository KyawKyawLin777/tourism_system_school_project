...
class Customer {
    ...
    public function emailExists() {
        try {
            $query = "SELECT id, full_name, email FROM " . $this->table_name . " WHERE email = ? LIMIT 0,1";
            $stmt = $this->conn->prepare($query);
    ...
    public function readOne() {
        $query = "SELECT id, full_name, email, phone, address, created_at FROM " . $this->table_name . " WHERE id = ? LIMIT 0,1";
        $stmt = $this->conn->prepare($query);
